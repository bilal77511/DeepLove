              ...
              <Disclosure.Panel className="flex flex-wrap w-full my-5 lg:hidden">
                <>
                  {navigation.map((item, index) => (
                    <Link key={index} href={item === "Demo" ? "https://deep-love-gen-r1.streamlit.app/" : "/"} target={item === "Demo" ? "_blank" : "_self"} className="w-full px-4 py-2 -ml-4 text-gray-500 rounded-md dark:text-gray-300 hover:text-pink-500 focus:text-pink-500 focus:bg-pink-100 dark:focus:bg-gray-800 focus:outline-none">
                      {item}
                    </Link>
                  ))}
          ...
          <ul className="items-center justify-end flex-1 pt-6 list-none lg:pt-0 lg:flex">
            {navigation.map((menu, index) => (
              <li className="mr-3 nav__item" key={index}>
                <Link 
                  href={menu === "Demo" ? "https://deep-love-gen-r1.streamlit.app/" : "/"} 
                  target={menu === "Demo" ? "_blank" : "_self"}
                  className="inline-block px-4 py-2 text-lg font-normal text-gray-800 no-underline rounded-md dark:text-gray-200 hover:text-pink-500 focus:text-pink-500 focus:bg-pink-100 focus:outline-none dark:focus:bg-gray-800"
                >
                  {menu}
                </Link>
              </li>
